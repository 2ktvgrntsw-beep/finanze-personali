...
export const renderEnergia = async (root) => {
  ...
  const annoCorr = anni[0];
  const aggAnno = aggregatoAnno(annoCorr);

  // KPI: costo/giorno, prezzo medio, spesa anno, consumo anno (con variazioni a/a)
  const vSpesa = variazioneAnno(annoCorr, 'spesa');
  const vCons = variazioneAnno(annoCorr, 'consumo');
  const vPrezzo = variazioneAnno(annoCorr, 'eurKwh');
  const vGiorno = variazioneAnno(annoCorr, 'eurGiorno');
  ...
  root.innerHTML = `
    ...
    <!-- KPI -->
    <div class="ekpis">
      <div class="ekpi"><div class="k">Costo/giorno</div><div class="v num">${k.eurGiorno != null ? fmtEUR(k.eurGiorno) : '—'}</div>${vGiorno != null ? `<div class="d ${_clsVar(vGiorno)}">${_fmtVar(vGiorno)} vs ${annoCorr - 1}</div>` : ''}</div>
      <div class="ekpi"><div class="k">Prezzo medio</div><div class="v num">${aggAnno.eurKwh.toFixed(3)}<small> €/kWh</small></div>${vPrezzo != null ? `<div class="d ${_clsVar(vPrezzo)}">${_fmtVar(vPrezzo)} vs ${annoCorr - 1}</div>` : ''}</div>
      <div class="ekpi"><div class="k">Spesa ${annoCorr}</div><div class="v num">${fmtEUR(aggAnno.spesa)}</div>${vSpesa != null ? `<div class="d ${_clsVar(vSpesa)}">${_fmtVar(vSpesa)} vs ${annoCorr - 1}</div>` : ''}</div>
      <div class="ekpi"><div class="k">Consumo ${annoCorr}</div><div class="v num">${aggAnno.consumo.toLocaleString('it-IT')}<small> kWh</small></div>${vCons != null ? `<div class="d ${_clsVar(vCons)}">${_fmtVar(vCons)} vs ${annoCorr - 1}</div>` : ''}</div>
    </div>
